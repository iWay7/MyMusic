...
public class ArrayHelper {
    ...
    public static short[] combine(short[]... arrays) {
        int totalLength = 0;
        for (short[] shorts : arrays) {
            totalLength += shorts.length;
        }
        short[] newArray = new short[totalLength];
        int pointer = 0;
        for (short[] shorts : arrays) {
            System.arraycopy(shorts, 0, newArray, pointer, shorts.length);
            pointer += shorts.length;
        }
        return newArray;
    }
